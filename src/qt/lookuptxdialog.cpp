    ...
    if (0<=populateResult) {
        ...
        switch(populateResult) {
            case MP_TX_NOT_FOUND:
                strText += "not a valid Ufobject or Omni transaction.  Please check the transaction hash "
            ...
            case MP_TX_IS_NOT_MASTER_PROTOCOL:
                strText += "a Ufobject transaction only.\n\nTip: You can use the debug console "
                           "'gettransaction' command to lookup specific Ufobject transactions.";
            break;
