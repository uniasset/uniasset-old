...
int mastercore_init()
{
    ...
    MYLOG("(6) load feature activation messages from txlistdb and process them accordingly");

    // load feature activation messages from txlistdb and process them accordingly
    p_txlistdb->LoadActivations(nWaterlineBlock);

    MYLOG("(7) load all alerts from levelDB (and immediately expire old ones)");

    // load all alerts from levelDB (and immediately expire old ones)
    p_txlistdb->LoadAlerts(nWaterlineBlock);

    MYLOG("(8) load the state of any freeable properties and frozen addresses from levelDB");
    ...
    MYLOG("(9 msc_initial_scan(nWaterlineBlock = %d );)", nWaterlineBlock);
...
bool CMPTxList::LoadFreezeState(int blockHeight)
{
    ...
    for (it->SeekToFirst(); it->Valid(); it->Next()) {
        ...
        PrintToConsole("DEBUG: loadOrder.push_back(%s,%16x)\n", sortKey, txid.GetCheapHash());
    }
    ...
    if ( txnsLoaded > 0 ) {
        PrintToConsole("txnsLoaded=%d\n", txnsLoaded );
    }

    // TODO: Make sure that we can safely rewrite the following paragraph for UFO blockchain.
    //
    // Seems like it is a code coverage protection of transaction loading from blockchain data.
    // Original bitcoin blockchain supposedly contains at least 2 initially (manually?) created txs
    // that are required to exist at block after 497000, they must be loaded and pass the assertion here.
    /*
