...
std::string mastercore::strMPProperty(uint32_t propertyId)
{
    ...
    if (0x80000000 & propertyId) {
        ...
        switch (propertyId) {
            case OMNI_PROPERTY_BTC: str = "UFO";
                break;
 ...
 */
static bool TXExodusFundraiser(const CTransactionRef& tx, const std::string& sender, int64_t amountInvested, int nBlock, unsigned int nTime)
{
 ...
 *   3 Class C (op-return)
 */
int mastercore::GetEncodingClass(const CTransactionRef& tx, int nBlock)
{
...
static bool FillTxInputCache(const CTransactionRef& tx)
{
...
static int parseTransaction(bool bRPConly, const CTransactionRef& wtx, int nBlock, unsigned int idx, CMPTransaction& mp_tx, unsigned int nTime)
{
    ...
    if (omniClass == OMNI_CLASS_A) {
        ...
        for (unsigned k = 0; k < script_data.size(); ++k) { // Step 1, locate the data packet
            ...
            if ("0000000000000001" == strSub || "0000000000000002" == strSub) { // peek & decode comparison
                if (strScriptData.empty()) { // confirm we have not already located a data address
                    ...
                    if (msc_debug_parser_data) PrintToLog("Multiple Data Addresses found (collision?) Class A invalidated, defaulting to UFO payment\n");
                    break;
                }
            }
        }
                        ...
                        for (unsigned int exodus_idx = 0; exodus_idx < ExodusValues.size(); exodus_idx++) {
                            if (value_data[k] == ExodusValues[exodus_idx]) { //this output matches data address value and exodus address value, choose as ref
                                if (strRefAddress.empty()) {
                                    ...
                                    strRefAddress.clear();
                                    if (msc_debug_parser_data) PrintToLog("Reference Address collision, multiple potential candidates. Class A invalidated, defaulting to UFO payment\n");
                                    break;
                                }
                            }
                        }
        ...
        if (!strDataAddress.empty()) { // valid Class A packet almost ready
            ...
            if ((!bRPConly || msc_debug_parser_readonly) && msc_debug_parser_dex) {
                ...
                PrintToLog("!! this may be the UFO payment for an offer !!\n");
            }
        }
    }
...
int ParseTransaction(const CTransactionRef& tx, int nBlock, unsigned int idx, CMPTransaction& mptx, unsigned int nTime)
{
...
static bool HandleDExPayments(const CTransactionRef& tx, int nBlock, const std::string& strSender)
{
    int count = 0;
...
static bool HandleExodusPurchase(const CTransactionRef& tx, int nBlock, const std::string& strSender, unsigned int nTime)
{
...
int mastercore_init()
{
    ...
    MYLOG("(5) load feature activation messages from txlistdb and process them accordingly");
    // load feature activation messages from txlistdb and process them accordingly
    p_txlistdb->LoadActivations(nWaterlineBlock);

    MYLOG("(6) load all alerts from levelDB (and immediately expire old ones)");
    // load all alerts from levelDB (and immediately expire old ones)
    p_txlistdb->LoadAlerts(nWaterlineBlock);


    MYLOG("(7) load the state of any freeable properties and frozen addresses from levelDB");
    ...
    MYLOG("(7) msc_initial_scan(nWaterlineBlock = %d )", nWaterlineBlock);
    // initial scan
...
// Omni Core On
bool mastercore_handler_block_rescan( const CBlockIndex* pindexNew, const CBlock& blockConnecting ) 
{
    ...
    //MYLOG("(3) mastercore_handler_block_begin(mastercore::GetHeight() = %d, pindexNew = %08x)", mastercore::GetHeight(), pindexNew );

    mastercore_handler_block_begin(mastercore::GetHeight(), pindexNew);


    // Tell wallet about transactions that went from mempool
    // about transactions that got confirmed:

    {
        /*
        MYLOG("(6) BOOST_FOREACH(const CTransactionRef &tx, pblock->vtx) ");
        MYLOG("(6.2)    blockConnecting = %016x", blockConnecting );

        MYLOG("(6.2)    blockConnecting IS ptr" );

        MYLOG("(6.3)    blockConnecting->vtx = %016x", blockConnecting.vtx );
        MYLOG("(6.4)    blockConnecting->vtx.size() = %d", blockConnecting.vtx.size() );
        */

        BOOST_FOREACH(const CTransactionRef &tx, blockConnecting.vtx ) {
...
////    PrintToConsole("ConnectTip: (5) mastercore_handler_block_end\n");
//    MYLOG("(7) Omni Core: end of block connect notification" );
...
bool mastercore_handler_tx(const CTransactionRef& tx, int nBlock, unsigned int idx, const CBlockIndex* pBlockIndex)
{
    //MYLOG("(1) Enter" );
    ...
    //MYLOG("(3) nBlock < nWaterlineBlock? return" );
    //MYLOG("(3.1)  nBlock              = %d", nBlock );
    //MYLOG("(3.2)  nWaterlineBlock     = %d", nWaterlineBlock );
    ...
    MYLOG("(3)  nBlock              = %d", nBlock );

    //MYLOG("(4) parseTransaction()") ;
    ...
    if (pop_ret >= 0) {
        ...
        if (isNonMainNet() || nBlock <= params.LAST_EXODUS_BLOCK) {
            MYLOG("(4.1) HandleExodusPurchase()") ;
    ...
    if (0 == pop_ret) {
        ...
        if (interp_ret != PKT_ERROR - 2) {
            ...
            MYLOG("(5.1)  pop_ret = %d", pop_ret );
            MYLOG("(5.1)  interp_ret = %d", interp_ret );
            MYLOG("(5.1)  writing to db hash '%s'", tx->GetHash().ToString() );
    ...
    if (fFoundTx)
        MYLOG("(5) FOUND OMNI TRANSACTIONS: fFoundTx = %d", fFoundTx ) ;
        ...
        unsigned int countMP)
{
    ...
    // TODO: Decide if UFO ACP is enough, and there is no need for Omnicore original checkpoints
    bool checkpointValid = VerifyCheckpoint(nBlockNow, pBlockIndex->GetBlockHash());
    if (/*TODO: remove false*/false && !checkpointValid) {
